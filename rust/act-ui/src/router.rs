...
//!
//! **Candidate for `common`.** A `CommandQueue` is single-consumer: the
//! first actuator to pop a command takes it whatever its target, so with
//! two actuators reading the same queue the speaker eats the UI's `attend`
//! and the UI eats the speaker's `say`. Something has to fan out by target,
//! and that something belongs next to the queue rather than inside one
//! actuator. Until it moves, this module is the one implementation and the
//! binary owns it.
//!
...
use common::{Command, CommandQueue};
